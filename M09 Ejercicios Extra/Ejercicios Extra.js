...
function deObjetoAarray(objeto) {
   ...
   // Tu código:
   let arrDeArr = Object.entries (objeto);
   return arrDeArr;
}
...
function capToFront(string) {
   ...
   for (let i = 0; i < newStr.length; i++) {
      if (newStr[i].toUpperCase() === newStr[i]) {
         upperArr.push(newStr[i]);
      } else lowerArr.push (newStr[i]);
   }
   finalArr = upperArr.concat (lowerArr);
...
function asAmirror(frase) {
   ...
   // Tu código:
   let fraseEspejo = frase.split('').reverse().join('').split(' ').reverse().join(' ');
   return fraseEspejo;
}
...
function deleteAbc(string) {
   ...
   let strFinal = arr.filter(e => !validacion.includes(e))
